      ...
      <div class="grid grid-cols-1 grid-rows-1 w-full h-full p-6 pt-28 md:p-10 xl:p-20 items-center justify-center">
         ...
         @foreach($products as $index => $product)
            <div class="product-wrapper col-start-1 row-start-1 w-full h-full grid grid-cols-1 xl:grid-cols-3 gap-4 md:gap-8 items-center max-w-full"
               ...
               <div class="flex flex-col h-full justify-start md:justify-end text-sm md:text-base order-1 xl:order-none relative z-20">
                  <p class="text-justify description max-w-[90%] md:max-w-sm text-pretty translate-y-10 opacity-0 text-text/80 leading-relaxed font-light">
                     {{ $product->description }}
                  </p>
               </div>
