...
export class CandleScene {
    ...
    loadModel() {
        ...
        const labelTexture = textureLoader.load(
            "/assets/textures/label-botanica.png",
        );
        ...
        loader.load("/assets/models/candle.glb", (gltf) => {
            ...
            gsap.from(this.candleContainer.rotation, {
                y: 0,
                duration: 2,
                ease: "power3.out",
            });
            gsap.from(this.candleContainer.position, {
                y: -3,
                duration: 1.5,
                ease: "expo.out",
                onComplete: () => {
    ...
    animate() {
        ...
        if (this.candleContainer) {
            this.candleContainer.position.y =
                -0.5 + Math.sin(Date.now() * 0.001) * 0.05;

            this.candleContainer.rotation.y += 0.005;

            const targetRotationX = Math.PI / 12 + this.mouseY * 0.1;
            this.candleContainer.rotation.x +=
                (targetRotationX - this.candleContainer.rotation.x) * 0.05;

            const targetRotationZ = Math.PI / 24 - this.mouseX * 0.05;
            this.candleContainer.rotation.z +=
                (targetRotationZ - this.candleContainer.rotation.z) * 0.05;
        }
