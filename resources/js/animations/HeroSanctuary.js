...
export class HeroSanctuary {
    ...
    init() {
        this.ctx = gsap.context(() => {
            ...
            const explanationText =
                heroSection.querySelector(".explanation-text");
            ...
            tl.fromTo(
                [button, explanationText, establisedText],
                { y: 20, opacity: 0 },
                {
                    y: 0,
                    opacity: 1,
            ...
            exitTl.to(title, { yPercent: -150, opacity: 0 }, 0);
            exitTl.to(
                [explanationText, establisedText, button],
                { y: -100, opacity: 0 },
                0,
            );
        }, this.scope);
    ...
    kill() {
        if (this.hoverInstances.length)
            this.hoverInstances.forEach((e) => {
                e.kill();
            });
        this.hoverInstances = [];
